...
entity streamToFinn_AXI_CPU_s_axi is
generic (
...
end entity streamToFinn_AXI_CPU_s_axi;
...
architecture behave of streamToFinn_AXI_CPU_s_axi is
    ...
    component streamToFinn_AXI_CPU_s_axi_ram is
        generic (
            BYTES   : INTEGER :=4;
            DEPTH   : INTEGER :=256;
            AWIDTH  : INTEGER :=8);
    ...
    end component streamToFinn_AXI_CPU_s_axi_ram;
...
-- ----------------------- Instantiation------------------
-- int_image_in
int_image_in : streamToFinn_AXI_CPU_s_axi_ram
generic map (
     BYTES    => 4,
     DEPTH    => 1536,
     AWIDTH   => log2(1536))
...
-- ----------------------- AXI READ ----------------------
    ...
    RRESP   <= "00";  -- OKAY
    RVALID_t  <= '1' when (rstate = rddata) and (int_image_in_read = '0') else '0';
    RVALID    <= RVALID_t;
...
entity streamToFinn_AXI_CPU_s_axi_ram is
    generic (
        BYTES   : INTEGER :=4;
        DEPTH   : INTEGER :=256;
        AWIDTH  : INTEGER :=8);
...
end entity streamToFinn_AXI_CPU_s_axi_ram;

architecture behave of streamToFinn_AXI_CPU_s_axi_ram is
